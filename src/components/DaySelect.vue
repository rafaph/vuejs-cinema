<template>
    <div id="day-select">
        <ul class="days">
            <li v-for="(day, index) in days" :key="index" :class="{ day: true, active: isActive(day) }" @click="selectDay(day)">
                {{ formatDay(day) }}
            </li>
            <li class="day-selector">
                <span class="dec" @click="changeDay(-1)" />
                <span class="inc" @click="changeDay(1)" />
            </li>
        </ul>
    </div>
</template>
